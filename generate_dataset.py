...
skeleton_pth = './sign_language_6_channels_results/'  # yy: path to save the generated dataset
dataset_6_channels_pth = './sign_language_results_hand_6_channels/'
dataset_3_channels_pth = './sign_language_results_hand_3_channels/'
...
for i, img in enumerate(sorted(os.listdir(img_folder_pth))[:SMAPLE_NUM]):
    ...
    #                 continue
        ...
        #     continue
        # try:
        points = np.load('./sign_language_One_Hand_preprocessed_generated_results/fingertips_' + file[:-4]  + '.npy', allow_pickle=True)
        handedness = np.load('./sign_language_One_Hand_preprocessed_generated_results/handedness_' + file[:-4] + '.npy')
        # print("Line 94")
        # except:
        #     continue
            # print(i)
        ...
        for i in np.flip(np.argsort(pose_pair_joint_locations)):
            ...
            # print(f' Part A:({points[partA][0]}, {points[partA][1]})')
            # print(f' Part B:({points[partB][0]}, {points[partB][1]})')
